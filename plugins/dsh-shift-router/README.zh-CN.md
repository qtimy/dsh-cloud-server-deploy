...
- **部署感知的子代理路由** —— 每个派生/分叉工作代理会独立判定为 `tiny`、`fast`、`code`、`smart`、`heavy` 或 `image`，再从 DSH 实际公布的 provider/model 目录中选择。认证/配置错误会停用整个 provider 并尝试下一候选；瞬时错误只停用当前模型。
- **订阅优先的成本策略** —— `opencode-go` 与 `qwen-token-plan-cn` 视为订阅路线。凡通过 DSH 可配置 provider 接口声明的自定义 provider，始终视为 **PAYG 自定义**，即使 ID 看似订阅路线。能力匹配优先，同等匹配下优先订阅。
...
| `subagents.enabled` | `true` | 按实时部署目录判定并路由 subagent/fork 工作代理 |
| `subagents.judgeTimeout` | `5000` | 子任务裁判每模型超时（毫秒） |
| `subagents.judgeMaxTokens` | `256` | 子任务裁判最大输出 token |
| `subagents.judgePromptCap` | `4000` | 发送给子任务裁判的最大字符数 |
| `subagents.catalogRefreshMs` | `300000` | provider/model 目录刷新间隔；适配器变化时立即刷新 |
| `subagents.verbose` | `false` | 记录子任务分类、计费类别与故障转移 |
...
**子代理会被独立路由。** `subagent` 或 `subagent_fork` 派生的工作代理带有 `session.header.origin === 'subagent'`；Shift-Router 会判定委派任务、从实时部署目录选模并使用有限回退链。显式固定的子模型保持不变；只有从父代理继承的默认模型会被替换。
...
- 默认情况下工作代理继承父代理模型。Shift-Router 会识别该继承值并替换为目录排序后的子代理路线；真正显式指定的子模型不会被覆盖。
- 子任务裁判遍历配置的 Fast 链，执行候选来自 DSH 当前 provider/model 目录，因此不再依赖独立 orchestrator 的硬编码模型表，并会跟随部署变化。
